...
def getDeployedSecretV2(dongle, masterPrivate, targetId, signerCertChain=None, ecdh_secret_format=None):
	...
	if (signerCertChain):
		...
		eprint("Using test master key %s " % binascii.hexlify(testMasterPublic))
	...
	eprint("Using ephemeral key %s" %binascii.hexlify(ephemeralPublic))
	...
	while True:
		...
		if not last_dev_pub_key.ecdsa_verify(bytes(certificateSignedData), certificateSignature):
			if index == 0:
				...
				eprint("Broken certificate chain - loading from user key")
			else:
